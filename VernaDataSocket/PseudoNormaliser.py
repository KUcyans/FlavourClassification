...
class PseudoNormaliser:
    ...
    def _log10_charge(
        self, features_np: np.ndarray, column_names: list[str]
    ) -> np.ndarray:
        ...
        for col_name in q_columns:
            if col_name in column_names:
                ...
                features_np[:, idx] = (
                    np.log10(np.clip(features_np[:, idx], a_min=1e-9, a_max=None))
                    - self.Q_shifter
                )
        return features_np

    def _pseudo_normalise_dom_pos(
        self, features_np: np.ndarray, column_names: list[str]
    ) -> np.ndarray:
        pos_columns = [
            "dom_x",
