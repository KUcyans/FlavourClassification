...
def get_nu_prob(
    df: pd.DataFrame, flavour: Flavour
    ...
    index = (
        ...
        else 1 if flavour == Flavour.MU else 2 if flavour == Flavour.TAU else None
    )
...
def plot_binary_flavour_ROC(df: pd.DataFrame, signal_flavour: Flavour, id: str) -> None:
    ...
    for flavour, output in all_outputs.items():
        y_true.append(
            np.ones_like(output) if flavour == signal_flavour else np.zeros_like(output)
        )
    ...
    residual_flavours = [
        flavour for flavour in all_outputs.keys() if flavour != signal_flavour
    ]
    label_roc = (
        rf"$\bf{signal_flavour.latex}$ vs "
        rf"(${residual_flavours[0].latex}$+${residual_flavours[1].latex}$)"
        + "\n"
        + f"AUC = {roc_auc:.3f}"
    )
    set_colour = (
        getColour(2)
        if signal_flavour == Flavour.E
        else getColour(0) if signal_flavour == Flavour.MU else getColour(1)
    )
    ...
    ax.set_title(
        rf"{id}: ${signal_flavour.latex}$ vs (${residual_flavours[0].latex}$+${residual_flavours[1].latex}$)"
    )
...
def plot_multi_flavour_ROC(df: pd.DataFrame, id: str) -> None:
    ...
    for colour_i, flavour in [(2, Flavour.E), (0, Flavour.MU), (1, Flavour.TAU)]:
        ...
        y_true = np.concatenate(
            [
                (
                    np.ones_like(nu_e_prob)
                    if flavour == Flavour.E
                    else np.zeros_like(nu_e_prob)
                ),
                (
                    np.ones_like(nu_mu_prob)
                    if flavour == Flavour.MU
                    else np.zeros_like(nu_mu_prob)
                ),
                (
                    np.ones_like(nu_tau_prob)
                    if flavour == Flavour.TAU
                    else np.zeros_like(nu_tau_prob)
                ),
            ]
        )
        ...
        ax.plot(
            fpr,
            tpr,
            label=rf"${flavour.latex}$ (AUC = {roc_auc:.3f})",
            linewidth=2,
            color=getColour(colour_i),
            markersize=1,
        )
...
def plot_prob_distribution(
    ...
    def build_label(probs, label_flavour):
        return (
            rf"$\bf{label_flavour.latex}$ (N={len(probs)})"
            + "\n"
            + rf"frac>{threshold_90:.2f} = "
            + rf"$\mathbf{{{fraction_above_threshold(probs, threshold_90):.3f}}}$"
            + "\n"
            + rf"frac>{threshold_80:.2f} = "
            + rf"$\mathbf{{{fraction_above_threshold(probs, threshold_80):.3f}}}$"
        )
    ...
    ax.set_title(rf"{id}: ${flavour.latex}$ Probability Score", fontsize=24)
    ax.set_xlabel(rf"${flavour.latex}$ Probability Score", fontsize=24)
...
def get_significance(flavour, total_e, total_mu, total_tau):
    ...
    poisson_label = rf"$Z_{{\mathrm{{Poisson}}, {flavour.latex}}}$"
    asimov_label = rf"$Z_{{\mathrm{{Asimov}}, {flavour.latex}}}$"

    poisson = (
        main_count / np.sqrt(main_count + opposite_count)
        if (main_count + opposite_count) > 0
        else 0
    )
    asimov = np.sqrt(
        2
        * (
            (main_count + opposite_count)
            * np.log(1 + main_count / (opposite_count + 1e-8))
            - main_count
        )
    )
...
def plot_prob_distribution_with_truncation(
    df: pd.DataFrame, flavour: Flavour, id: str, magnifier: Tuple[float, float] = None
) -> None:
    nu_e_prob, nu_mu_prob, nu_tau_prob = get_nu_prob(df, flavour)
    Nbins, binwidth, bins, counts, bin_centers = getHistoParam(
        nu_tau_prob, binwidth=0.005
    ...
    def get_union_annotation(indices):
        ...
        poisson, asimov, poisson_label, asimov_label = get_significance(
            flavour, total_e, total_mu, total_tau
        )
        # significance, significance_label = get_significance(flavour, total_e, total_mu, total_tau)

        lines = [
            rf"${Flavour.E.latex}$ {bin_range} = {total_e}",
            rf"${Flavour.MU.latex}$ {bin_range} = {total_mu}",
            rf"${Flavour.TAU.latex}$ {bin_range} = {total_tau}",
            # fr"{significance_label} = {significance:.2f}"
            # fr"{poisson_label} = {poisson:.2f}",
            rf"{asimov_label} = {asimov:.2f}",
        ]
    ...
    if not magnifier:
        ...
        if magnifier_bins.size > 0:
            ...
            magnifier_max_bin_idx = magnifier_bins[
                np.argmax(hist_stack[:, magnifier_bins].sum(axis=0))
            ]
    ...
    ax.set_title(rf"{id}: ${flavour.latex}$ Probability Score")
    ax.set_xlabel(rf"${flavour.latex}$ Probability Score")
    ax.set_ylabel("Frequency")
    ax.legend(fontsize=24, loc="upper center")
    if magnifier:
...
def plot_log_energy_prob_distribution_mono_flavour(
    nu_prob: np.ndarray,
    energy: np.ndarray,
    output_flavour: Flavour,
    flavour: Flavour,
    id: str,
    normalise_by_energy: bool = True,
) -> None:
    Nbins, binwidth, bins, counts, bin_centers = getHistoParam(nu_prob, binwidth=0.1)
    log_energy_binwidth = 0.25
    log_energy = np.log10(energy)
    log_energy_bin = np.arange(
        np.min(log_energy), np.max(log_energy), log_energy_binwidth
    )

    # Swapped axes: Energy (x), Logit (y)
    raw_counts, xedges, yedges = np.histogram2d(
        log_energy, nu_prob, bins=[log_energy_bin, bins]
    )
    H = raw_counts.copy()
    ...
    fig.colorbar(
        ...
        label=(
            "Frequency"
            if not normalise_by_energy
            else "Frequency normalised within energy bin"
        ),
    )

    ax.set_title(
        rf"{id}: log₁₀(Energy [GeV])–Prob ${output_flavour.latex}$ of true ${flavour.latex}$",
        fontsize=24,
    )
    ax.set_xlabel("log₁₀(Energy [GeV])", fontsize=24)
    ax.set_ylabel(rf"${flavour.latex}$ Probability Score", fontsize=24)
    # ax.set_ylim(-0.1, 1.1)
...
def plot_energy_distribution(
    df: pd.DataFrame, flavour: Flavour, true_flavour: Flavour, id: str
) -> None:
    nu_e_prob, nu_mu_prob, nu_tau_prob = get_nu_prob(df, flavour)
    e_energy, mu_energy, tau_energy = get_energy(df)
    if true_flavour == Flavour.E:
        plot_log_energy_prob_distribution_mono_flavour(
            nu_e_prob, e_energy, flavour, Flavour.E, id, normalise_by_energy=True
        )
    elif true_flavour == Flavour.MU:
        plot_log_energy_prob_distribution_mono_flavour(
            nu_mu_prob, mu_energy, flavour, Flavour.MU, id, normalise_by_energy=True
        )
    elif true_flavour == Flavour.TAU:
        plot_log_energy_prob_distribution_mono_flavour(
            nu_tau_prob, tau_energy, flavour, Flavour.TAU, id, normalise_by_energy=True
        )


def plot_zenith_prob_distribution_mono_flavour(
    nu_prob: np.ndarray,
    ...
    raw_counts, xedges, yedges = np.histogram2d(
        nu_zenith, nu_prob, bins=[zenith_bin, bins]
    )
    H = raw_counts / (
        raw_counts.sum(axis=1, keepdims=True) + 1e-8
    )  # normalise by zenith bin (now rows)
    ...
    ax.set_title(
        rf"{id}: Zenith–Prob ${output_flavour.latex}$ of true ${flavour.latex}$",
        fontsize=24,
    )
...
def plot_zenith_distribution(
    ...
    if true_flavour == Flavour.E:
        plot_zenith_prob_distribution_mono_flavour(
            nu_e_prob, e_zenith, flavour, Flavour.E, id
        )
    elif true_flavour == Flavour.MU:
        plot_zenith_prob_distribution_mono_flavour(
            nu_mu_prob, mu_zenith, flavour, Flavour.MU, id
        )
    elif true_flavour == Flavour.TAU:
        plot_zenith_prob_distribution_mono_flavour(
            nu_tau_prob, tau_zenith, flavour, Flavour.TAU, id
        )


def extend_extract_metrics_for_all_flavours(
    df: pd.DataFrame, run_id: str, epoch: int
) -> dict:
    # build id with run_id and epoch
    metrics = {"runID": run_id, "epoch": epoch}
    ...
    for true_flavour in Flavour:
        ...
        metrics.update(
            {
                ...
                f"frac_pred_{true_flavour.alias}_on_true_{true_flavour.alias}_prob_gt_0.9": fraction_above_threshold(
                    target_prob, 0.9
                ),
                f"frac_pred_{true_flavour.alias}_on_true_{true_flavour.alias}_prob_gt_0.8": fraction_above_threshold(
                    target_prob, 0.8
                ),
                f"frac_pred_{true_flavour.alias}_on_true_{true_flavour.alias}_prob_lt_0.2": fraction_below_threshold(
                    target_prob, 0.2
                ),
                f"frac_pred_{true_flavour.alias}_on_true_{true_flavour.alias}_prob_lt_0.1": fraction_below_threshold(
                    target_prob, 0.1
                ),
                f"median_pred_{true_flavour.alias}_on_true_{true_flavour.alias}_prob": float(
                    np.median(target_prob)
                ),
            }
        )
        ...
        for pred_flavour in Flavour:
            ...
            metrics.update(
                {
                    f"frac_pred_{pred_flavour.alias}_on_true_{true_flavour.alias}_prob_gt_0.9": fraction_above_threshold(
                        pred_probs, 0.9
                    ),
                    f"frac_pred_{pred_flavour.alias}_on_true_{true_flavour.alias}_prob_gt_0.8": fraction_above_threshold(
                        pred_probs, 0.8
                    ),
                    f"median_pred_{pred_flavour.alias}_on_true_{true_flavour.alias}_prob": float(
                        np.median(pred_probs)
                    ),
                }
            )
