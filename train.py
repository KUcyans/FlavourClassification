...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Training Script with Timestamped Logs"
    )
    parser.add_argument(
...
def build_callbacks(config: dict, callback_dir: str):
    ...
    callbacks = [
        EarlyStopping(monitor="val_loss", patience=config["patience"], verbose=True),
        checkpoint_loss,
        checkpoint_mid,
...
def setup_directories(
    base_dir: str, config_dir: str, current_date: str, current_time: str
):
    ...
    paths = {
        ...
        "checkpoint_dir": os.path.join(
            base_dir, "checkpoints", current_date, current_time
        ),
...
def build_optimiser_and_scheduler(
    config: dict, model: torch.nn.Module, datamodule: MultiFlavourDataModule
):
    ...
    # Example of how to use other schedulers if needed:
...
def run_training(
    ...
    config_copy_path = os.path.join(
        dirs["config_history"], f"{current_date}_{current_time}_config.json"
    )
    ...
    optimiser_and_scheduler = build_optimiser_and_scheduler(
...
if __name__ == "__main__":
    ...
    end_time = time.time()
    print(
        f"Training completed in {time.strftime('%d:%H:%M:%S', time.gmtime(end_time - start_time))}"
    )
