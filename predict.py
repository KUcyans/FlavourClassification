...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Prediction Script with Timestamped Logs"
    )
    parser.add_argument(
        "--date", type=str, required=True, help="Execution date in YYYYMMDD format"
    )
    parser.add_argument(
        "--time", type=str, required=True, help="Execution time in HHMMSS format"
    )
    parser.add_argument(
        "--checkpoint_date",
        type=str,
        required=True,
        help="Date of the checkpoint in YYYYMMDD format",
    )
    parser.add_argument(
        "--checkpoint_time",
        type=str,
        required=True,
        help="Time of the checkpoint in HHMMSS format",
    )
    # add an optionl argument --runID where it can be empty
    parser.add_argument(
        "--runID", type=str, default="", help="Run ID for the prediction"
    )
...
def setup_directories(
    ...
    paths = {
        ...
        "predict_dir": os.path.join(
            ...
            "predictions",
            current_date,
            f"model_{checkpoint_date}_{checkpoint_time}",
            current_time,
        ),
        "checkpoint_dir": os.path.join(
            base_dir, "checkpoints", checkpoint_date, checkpoint_time
        ),
    ...
    return {
        **paths,
        "predict_log_file": os.path.join(
            paths["log_dir"], f"{current_time}_predict.log"
        ),
    }
...
def load_model_config(dirs, checkpoint_date, checkpoint_time):
    config_file = os.path.join(
        dirs["config_history"], f"{checkpoint_date}_{checkpoint_time}_config.json"
    )
...
def build_model(config: dict, device: torch.device, ckpt_file: str):
    ...
    positional_encoding_type = PositionalEncodingType.from_string(
...
def build_data_module(
    config: dict, er: EnergyRange, root_dir: str, root_dir_corsika: str = None
):
...
def build_predictions(
    config: dict, predictions: list, prediction_dir: str, ckpt_file: str
):
    all_preds = {
        "target_class": [],
        "pred_class": [],
    ...
    num_class = ClassificationMode.from_string(
    ...
    for i, batch in enumerate(predictions):
        ...
        pred_one_hot = torch.nn.functional.one_hot(
            pred_class, num_classes=num_class
        ).tolist()
        target_one_hot = torch.nn.functional.one_hot(
            target_class, num_classes=num_class
        ).tolist()
...
def save_predictions(
    ...
    if val_name == "val_loss":
        csv_name = os.path.join(
            prediction_dir, f"predictions_epoch_{epoch}_val_loss_{val_value}.csv"
        )
    elif val_name == "val_tau_lg_085_tau":
        csv_name = os.path.join(
            prediction_dir, f"predictions_epoch_{epoch}_tau085_{val_value}.csv"
        )
    else:
    ...
    if len(df_predictions) != len(df_analysis):
        raise ValueError(
            f"Mismatch: {len(df_predictions)} predictions vs {len(df_analysis)} analysis rows."
        )
...
def run_prediction(
    ...
    for ckpt_file in ckpt_files:
        ...
        try:
            ...
            df_predictions = build_predictions(
                config=config,
            ...
            plot_all_metrics(
                df_combined, pdf_path=pdf_file, run_id=model_id, epoch=epoch
            )
            try:
                metrics = extend_extract_metrics_for_all_flavours(
                    df_combined, run_id=model_id, epoch=epoch
                )
